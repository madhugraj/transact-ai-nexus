...
export const ComparisonResultsPanel: React.FC<ComparisonResultsPanelProps> = ({
  ...
  const safeLineItems = Array.isArray(lineItems) ? lineItems.filter(i => i && typeof i === "object" && i.id) : [];
  const safeComparisonResults = Array.isArray(comparisonResults) ? comparisonResults.filter(r => r && typeof r.match !== "undefined") : [];
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-lg flex justify-between items-center">
            <span>Document Comparison Summary</span>
            <span className={`${matchPercentage > 80 ? 'text-green-600' : 'text-red-600'} text-2xl font-bold`}>
              {matchPercentage}% Match
            </span>
          </CardTitle>
            ...
            <>
              ...
              <div className="mt-6 grid md:grid-cols-3 gap-4">
                <div className="bg-blue-50 p-4 rounded-md">
                  <div className="text-sm font-medium text-blue-600">PO Total</div>
                  <div className="text-2xl font-bold">${poTotal.toFixed(2)}</div>
                </div>
                <div className="bg-amber-50 p-4 rounded-md">
                  <div className="text-sm font-medium text-amber-600">Invoice Total</div>
                  <div className="text-2xl font-bold">${invoiceTotal.toFixed(2)}</div>
                </div>
                <div className={`${percentageDifference > 5 ? 'bg-red-50' : 'bg-green-50'} p-4 rounded-md`}>
                  <div className={`text-sm font-medium ${percentageDifference > 5 ? 'text-red-600' : 'text-green-600'}`}>
                    Difference
                  </div>
              ...
              {/* Line Items Table */}
              <div className="mt-6">
                <h3 className="text-md font-semibold mb-2">Line Item Comparison</h3>
                <div className="overflow-x-auto">
                  <Table>
                    <TableHeader>
                      <TableRow>
